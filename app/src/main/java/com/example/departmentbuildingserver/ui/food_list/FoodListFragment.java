package com.example.departmentbuildingserver.ui.food_list;
...
import com.example.departmentbuildingserver.SizeAddOnEditActivity;
import com.example.departmentbuildingserver.adapter.MyFoodListAdapter;
import com.example.departmentbuildingserver.common.Common;
import com.example.departmentbuildingserver.common.MySwiperHelper;
import com.example.departmentbuildingserver.databinding.FragmentFoodListBinding;
import com.example.departmentbuildingserver.model.FoodModel;
...
public class FoodListFragment extends Fragment {
    ...
    @BindView(R.id.imgAddNewPost)
    ImageView imgAddNewPost;
    ...
    private void initListener() {
        imgAddNewPost.setOnClickListener(new View.OnClickListener() {
            @Override
    ...
    private void showCreateDialog() {
        ...
        builder.setNegativeButton("CANCEL", (((dialogInterface, i) -> {
                ...
                .setPositiveButton("CREATE", (((dialogInterface, i) -> {
                    FoodModel foodModel = new FoodModel(edt_food_name.getText().toString(),
                            Long.parseLong(edt_food_price.getText().toString()),
                            edt_food_description.getText().toString());
                    if(imageUri != null) {
                        //Have image
                        dialog.setMessage("Uploading...");
                        dialog.show();
                        ...
                        imageFolder.putFile(imageUri)
                                .addOnFailureListener(e->{
                                    ...
                                    imageFolder.getDownloadUrl().addOnSuccessListener(uri->{
                                        ...
                                        Common.categorySelected.getFoods().add(foodModel);
                                        addNewPost(Common.categorySelected.getFoods(), false);
                                    });
                                }).addOnProgressListener(taskSnapshot ->{
                                    double progress = (100.0*taskSnapshot.getBytesTransferred() / taskSnapshot.getTotalByteCount());
                                    dialog.setMessage(new StringBuilder("Uploading: ").append(progress).append("%"));
    ...
    private void addNewPost(List<FoodModel> foods, boolean isDelete) {
